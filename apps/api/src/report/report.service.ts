...
export class ReportService {
  ...
  async dashboardStats(): Promise<Q> {
    ...
    // Previous week stock for trend (7 days ago snapshot from ledger balance)
    const prevWeekStock = await db.query(
    ...
    return {
      kpi: {
        ...
        total_pallets_utilization: 0, // Will be calculated from location data
        ...
        // Calculate location utilization from stockByLocation
  ...
  // ---------------------------------------------------------------------------
  // Dashboard FEFO Queue
  // ---------------------------------------------------------------------------

  async fefoQueue(limit = 50): Promise<Q> {
    // Pure FEFO-ordered stock: earliest expiry date first (most expired first),
    // regardless of how close to today it is.
    ...
    // Summary grouped by expiry year (most expired year first)
  ...
  async dashboardAlerts(): Promise<Q> {
    const db = this.db;

    // Expired items
    const expired = await db.query(
      `SELECT COUNT(*)::int as count, SUM(quantity) as qty
       FROM stock 
    ...
    const stocktakeLocks = await db.query(
      `SELECT COUNT(DISTINCT location)::int as locked_locations
    ...
    // Failed picks (outbound in Picking status for > 24 hours)
    const stalePicks = await db.query(
    ...
    // Overdue inbound (Dues In for > 7 days)
    const overdueInbound = await db.query(
      `SELECT COUNT(*)::int as count
       FROM inbound_orders
  ...
  async dashboardInsights(): Promise<Q> {
    const db = this.db;

    // Fast movers: products with high outbound volume in last 30 days
    ...
    // Slow movers: products with stock older than 90 days
    ...
    // Low stock: products below reorder point (arbitrary threshold: < 100 units)
    ...
    const locationUtil = await db.query(
      `SELECT 
        lm.aisle,
