...
/**
 * Parse and update location_master table to populate rack, row_name fields
 * This is a helper for database migration/update
 */
export function parseLocationForDB(locationCode: string): { 
  rack: string | null; 
  ...
  if (!parsed.isValid) {
    return { rack: null, row_name: null, position: null, aisle: null };
  }

  return {
    rack: parsed.fullRack,     // CD01
    row_name: parsed.level,     // A
    position: parsed.position,  // 02
    aisle: parsed.rack,         // CD (aisle corresponds to rack identifier)
  };
}
