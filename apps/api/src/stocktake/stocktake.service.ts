...
import { todayStr, todayCompact } from '../common/date-util';
...
export class StockTakeService {
  ...
  async getItems(stockTakeId: number): Promise<any[]> {
    const r = await this.db.query(
  ...
  async calculateAccuracy(stockTakeId: number): Promise<Q> {
    ...
    for (const item of items) {
      ...
      if (item.status === 'Plus') plus += Math.abs(Number(item.difference ?? 0));
      else if (item.status === 'Minus') minus += Math.abs(Number(item.difference ?? 0));
      else clear += Number(item.qty_physical ?? 0);
    }
  ...
  async addItemFull(stockTakeId: number, data: Q): Promise<number> {
  ...
  async getSystemStock(productId: number, location?: string | null, batchNumber?: string | null): Promise<number> {
  ...
  async getActiveLockedLocations(): Promise<string[]> {
    const r = await this.db.query(
  ...
  async getScopeLocations(): Promise<{ locations: string[]; locked: string[] }> {
  ...
  async getStock(productId: number, location?: string | null, batch?: string | null): Promise<Q> {
  ...
  // ---------------------------------------------------------------------------
  // Counting / review / adjustment flow
  // ---------------------------------------------------------------------------

  async startCounting(id: number): Promise<void> {
    const st = await this.getById(id);
    if (!st) throw ApiException.badRequest('Stock take tidak ditemukan');
    if (st.status !== 'Draft') throw ApiException.badRequest('Status harus Draft untuk memulai Counting');
    await this.db.query(`UPDATE stock_take SET status='Counting', counting_round='c1', updated_at=NOW() WHERE id=$1`, [id]);
  }

  private async saveC1(id: number, values: Record<string, any>): Promise<void> {
    const st = await this.getById(id);
    if (!st) throw ApiException.badRequest('Stock take tidak ditemukan');
    if (st.status !== 'Counting' || st.counting_round !== 'c1') {
      throw ApiException.badRequest('Tidak bisa simpan Counter 1 — bukan giliran C1');
    }
  ...
  async advanceToC2(id: number, c1Values: Record<string, any>): Promise<void> {
    const st = await this.getById(id);
    if (!st) throw ApiException.badRequest('Stock take tidak ditemukan');
    if (st.status !== 'Counting' || st.counting_round !== 'c1') {
      throw ApiException.badRequest('Harus di tahap Counter 1 untuk maju ke Counter 2');
    }
    await this.saveC1(id, c1Values);
    await this.db.query(`UPDATE stock_take SET counting_round='c2', updated_at=NOW() WHERE id=$1`, [id]);
  }

  async saveCounters(id: number, counters: Record<string, { c1?: any; c2?: any; c3?: any }>): Promise<void> {
    for (const [itemId, v] of Object.entries(counters)) {
      ...
      await this.db.query(
        'UPDATE stock_take_items SET counter_1=$1, counter_2=$2, counter_3=$3 WHERE id=$4 AND stock_take_id=$5',
        [c1, c2, c3, Number(itemId), id],
      );
    }
  }

  async finishCounting(id: number, c2Values: Record<string, any>): Promise<void> {
    await this.db.transaction(async (client) => {
      const st = await this.getById(id);
      if (!st) throw ApiException.badRequest('Stock take tidak ditemukan');
      if (st.status !== 'Counting') throw ApiException.badRequest('Status harus Counting');
      if (st.counting_round !== 'c2') {
        throw ApiException.badRequest('Counter 1 belum selesai — selesaikan Counter 1 dulu');
      }
  ...
  private async saveC2WithClient(client: any, id: number, values: Record<string, any>): Promise<void> {
    for (const [itemId, val] of Object.entries(values)) {
  ...
  async saveReview(id: number, physicals: Record<string, number | string>): Promise<void> {
    const st = await this.getById(id);
    if (!st) throw ApiException.badRequest('Stock take tidak ditemukan');
    if (st.status !== 'Review') throw ApiException.badRequest('Status harus Review');
    for (const [itemId, qtyPhysicalRaw] of Object.entries(physicals)) {
      const itemIdNum = Number(itemId);
      const getStmt = await this.db.query(
        'SELECT qty_system FROM stock_take_items WHERE id=$1 AND stock_take_id=$2',
        [itemIdNum, id],
      );
      ...
      const status = difference > 0.001 ? 'Plus' : difference < -0.001 ? 'Minus' : 'Clear';
      await this.db.query(
        'UPDATE stock_take_items SET qty_physical=$1, difference=$2, status=$3 WHERE id=$4 AND stock_take_id=$5',
        [qtyPhysical, difference, status, itemIdNum, id],
      );
    }
  }

  async applyAdjustment(id: number): Promise<void> {
    await this.db.transaction(async (client) => {
      const st = await this.getById(id);
      if (!st) throw ApiException.badRequest('Stock take tidak ditemukan');
      if (st.status !== 'Review') throw ApiException.badRequest('Status harus Review untuk apply adjustment');
      ...
      for (const item of items.rows) {
        ...
        if (stockRow) {
          ...
          await client.query(
            `INSERT INTO stock (product_id, batch_number, location, quantity, uom, stock_status)
             VALUES ($1,$2,$3,$4,$5,'Available')`,
            [productId, batch, location, qtyPhysical, uom],
          );
        }

        const balStmt = await client.query(
          'SELECT balance FROM stock_ledger WHERE product_id=$1 ORDER BY id DESC LIMIT 1',
          [productId],
        );
