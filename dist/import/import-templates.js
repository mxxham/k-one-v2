...
exports.tplInbound = tplInbound;
exports.tplOutbound = tplOutbound;
exports.tplStock = tplStock;
...
async function tplHeader(ws, headers) {
...
function tplSampleRow(ws, values) {
...
function tplNote(ws, label, value = '') {
    const rowNum = ws.rowCount + 1;
...
async function tplStock() {
    ...
    const descriptions = ['Kode produk (wajib)', 'No batch / lot', 'Lokasi bin (mis: A-01-01)', 'Jumlah qty (wajib)', 'Drum/Carton/Pail/Bags/EA (default: Drum)', 'Tgl produksi', 'Tgl exp', 'Available/Dues In/Reserved', 'Catatan opsional'];
    for (let i = 0; i < descriptions.length; i++)
        ws.getCell(2, i + 1).value = descriptions[i];
    const examples = ['SHE-001', 'BT2024001', 'A-01-01', 20, 'Drum', '01/01/2024', '01/01/2026', 'Available', 'Opening stock'];
    for (let i = 0; i < examples.length; i++)
        ws.getCell(3, i + 1).value = examples[i];
...
//# sourceMappingURL=import-templates.js.map