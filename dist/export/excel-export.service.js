...
function nowStamp() {
    return (0, date_util_1.todayStr)().replace(/-/g, '') + '_' + new Date().toISOString().slice(11, 19).replace(/:/g, '');
}
...
let ExcelExportService = class ExcelExportService {
    ...
    async inboundReport(status) {
        ...
        if (orders.length === 0)
            throw api_exception_1.ApiException.badRequest('Tidak ada data inbound untuk di-export.');
        const orderIds = orders.map((o) => Number(o.id));
        const r = await this.db.query(`SELECT ii.*,
       ...
       ORDER BY io.id, ii.id`, [orderIds]);
        ...
        for (const item of items) {
            ...
            if (inpValue === 'ATP')
                inpCell.font = { ...inpCell.font, color: { argb: 'FF166534' } };
            else if (inpValue === 'Dues In')
                inpCell.font = { ...inpCell.font, color: { argb: 'FF1E40AF' } };
            else if (inpValue === 'Unserviceable')
                inpCell.font = { ...inpCell.font, color: { argb: 'FF991B1B' } };
            if (dl !== '') {
                const dayCell = sheet.getCell(row, 17);
                if (dl < 0)
                    dayCell.font = { ...dayCell.font, color: { argb: 'FF991B1B' } };
                else if (dl <= 90)
                    dayCell.font = { ...dayCell.font, color: { argb: 'FF92400E' } };
            }
            row++;
        }
        ...
        void totalRange;
        for (let c = 1; c <= totalCols; c++)
            sheet.getColumn(c).width = c === 9 ? 26 : 14;
        return { buffer: await toBuffer(wb), filename: `Inbound_Report_${nowStamp()}.xlsx`, contentType: XLSX_TYPE };
    }
    async outboundReport(status) {
        const orders = await this.outbound.getAll(status, 2000, 0, null);
        if (orders.length === 0)
            throw api_exception_1.ApiException.badRequest('Tidak ada data outbound untuk di-export.');
        const orderIds = orders.map((o) => Number(o.id));
        const r = await this.db.query(`SELECT oi.*,
       ...
       ORDER BY o.order_date ASC, o.id ASC, COALESCE(od.seq,0) ASC, oi.id ASC`, [orderIds]);
        ...
        for (const orderId of Object.keys(groups)) {
            ...
            items.forEach((item, idx) => {
                ...
                const rowData = {
                    ...
                    8: item.uom ?? '',
                    9: item.dest_name ?? '',
                    10: String(((item.customer_id ?? '') !== '' ? `ID ${item.customer_id} | ` : '') +
                ...
                if (dl !== '') {
                    ...
                    if (dl < 0)
                        ...
                        dayCell.font = { ...dayCell.font, color: { argb: 'FF166534' } };
                }
                row++;
            });
            sheet.getCell(`A${row}`).value = 'TOTAL';
            ...
            if (groupVolume > 0)
                sheet.getCell(colLetter(16) + row).value = Math.round(groupVolume * 100) / 100;
            for (let c = 1; c <= totalCols; c++) {
                const cell = sheet.getCell(row, c);
                cell.font = { bold: true, size: 9, color: { argb: 'FFFFFFFF' } };
    ...
    async ledgerReport() {
        ...
        const r = await this.db.query(`SELECT sl.*,
       ...
       ORDER BY sl.transaction_date ASC, sl.created_at ASC`, [startDate, endDate]);
    ...
    async reportsExcel(type, date, dateTo) {
        if (type === 'stock') {
        ...
        (report.stock_summary ?? []).forEach((item, i) => {
            const row = 4 + i;
    ...
    async expiring365() {
        const r = await this.db.query(`SELECT s.*, p.product_code, p.product_name, p.uom_per_pallet,
    ...
    async stockReport() {
        const r = await this.db.query(`SELECT p.id AS product_id, p.product_code, p.product_name, p.category,
       ...
       ORDER BY p.product_name, s.batch_number, s.location`);
        ...
        for (const item of rows) {
            ...
            if (expPlt <= 0)
                expPlt = Math.ceil(qty / Math.max(1, Number(item.uom_per_pallet ?? 4)));
            ...
            if (info.is_expired) {
                for (let c = 1; c <= 16; c++)
                    sheet.getCell(row, c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFD7D7' } };
            }
            else if (info.is_critical) {
                for (let c = 1; c <= 16; c++)
                    sheet.getCell(row, c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF3CD' } };
            }
            row++;
        }
        sheet.getCell(`A${row}`).value = 'TOTAL';
    ...
    async stocktakeReport(id) {
        ...
        items.forEach((item, i) => {
            ...
            if (diff !== 0) {
                for (let c = 1; c <= 12; c++)
                    ws.getCell(row, c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF7ED' } };
            }
        });
