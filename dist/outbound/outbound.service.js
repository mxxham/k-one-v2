...
let OutboundService = class OutboundService {
    ...
    async generateNumber() {
        return (0, number_gen_1.generateNumber)(this.db, {
            ...
            column: 'order_number',
            prefix: `OUT-${(0, date_util_1.todayStr)().slice(0, 7).replace('-', '')}-`,
            searchPrefix: `OUT-${(0, date_util_1.todayStr)().slice(0, 7).replace('-', '')}-`,
            pad: 4,
        });
    }
    async saveDestinations(outboundId, names, locations, streets, kotas, notes) {
        const db = this.db;
        ...
        for (let i = 0; i < names.length; i++) {
            ...
            if (name === '')
                continue;
            await db.query(`INSERT INTO outbound_destinations (outbound_id, seq, ship_to_name, ship_to_location, ship_to_street, kota, notes)
    ...
    async attachDestination(outboundId, newItemId, item) {
        ...
        if (itemShipToName === '' && itemShipToLoc === '')
            return;
        const db = this.db;
        const last = await db.query('SELECT id FROM outbound_items WHERE outbound_order_id = $1 ORDER BY id DESC LIMIT 1', [outboundId]);
        const lastItemId = last.rows[0]?.id ?? null;
        const seqR = await db.query('SELECT COALESCE(MAX(seq),0)+1 as next_seq FROM outbound_destinations WHERE outbound_id = $1', [outboundId]);
        const nextSeq = Number(seqR.rows[0].next_seq ?? 1);
        const existsR = await db.query('SELECT id FROM outbound_destinations WHERE outbound_id = $1 AND ship_to_name = $2 LIMIT 1', [outboundId, itemShipToName]);
        const existing = existsR.rows[0];
        try {
            if (!existing) {
                const ins = await db.query(`INSERT INTO outbound_destinations (outbound_id, seq, ship_to_name, ship_to_location, kota, ship_to_street, notes)
           VALUES ($1,$2,$3,$4,$4,$5,NULL) RETURNING id`, [outboundId, nextSeq, itemShipToName, itemShipToLoc, itemShipToStreet]);
    ...
    async countAll(status, odNo) {
        ...
        const where = conditions.length ? 'WHERE ' + conditions.join(' AND ') : '';
        const r = await this.db.query(`SELECT COUNT(DISTINCT o.id)::int FROM outbound_orders o
       LEFT JOIN outbound_items oi ON o.id = oi.outbound_order_id ${where}`, params);
        return r.rows[0].count;
    }
    async getById(id) {
        const r = await this.db.query(`SELECT o.*,
    ...
    async getItems(outboundId) {
        const rowsR = await this.db.query(`SELECT oi.*,
       ...
       ORDER BY oi.id`, [outboundId]);
        ...
        for (const row of rows) {
            if (!row.resolved_batch) {
                const fefo = await this.db.query(`SELECT batch_number, location, expiry_date
           FROM stock
    ...
    async getItemPickedLocations(outboundItemId) {
        const r = await this.db.query(`SELECT oil.quantity AS picked_qty,
       ...
       ORDER BY COALESCE(sl.location_code, oi.location), sl.pallet_seq`, [outboundItemId]);
        return r.rows;
    }
    async getDestinations(outboundId) {
        const r = await this.db.query('SELECT * FROM outbound_destinations WHERE outbound_id = $1 ORDER BY seq', [outboundId]);
        return r.rows;
    }
    async getAvailableStock(productId, location) {
        ...
        const r = await this.db.query(`SELECT st.id, st.product_id, st.batch_number, st.location,
    ...
    async getTotalAvailableQty(productId) {
    ...
    async getFEFOAllocation(productId, requiredQty, location = null) {
    ...
    async checkStock(productId, quantity, location = null) {
    ...
    async addItemWithFEFO(outboundId, item) {
        ...
        if (quantity > totalAvailable) {
            throw new api_exception_1.ApiException('Stok tidak mencukupi. Stok tersedia: ' + Math.round(totalAvailable).toLocaleString('en-US') +
                ', Qty diminta: ' + Math.round(quantity).toLocaleString('en-US'), 409);
        }
        let fefo;
        if (manualLocs) {
            const manualTotal = manualLocs.reduce((a, b) => a + Number(b.qty ?? 0), 0);
            if (Math.abs(manualTotal - quantity) > 0.01) {
                throw new api_exception_1.ApiException('Total qty bin manual (' + Math.round(manualTotal).toLocaleString('en-US') +
                    ') tidak sama dengan qty order (' + Math.round(quantity).toLocaleString('en-US') + ')', 409);
            }
            ...
            for (const binEntry of manualLocs) {
                ...
                if (!binFefo.sufficient) {
                    throw new api_exception_1.ApiException(`Stok di bin '${binLoc}' tidak mencukupi. Diminta: ${Math.round(binQty).toLocaleString('en-US')}, Tersedia: ${Math.round(binFefo.total_available).toLocaleString('en-US')}`, 409);
                }
            ...
            if (!fefo.sufficient) {
                ...
                throw new api_exception_1.ApiException(msg, 409);
            }
        }
        ...
        if (manualLocs && manualLocs.length > 1) {
            locSummary = manualLocs.map((b) => b.location).join(', ');
        }
        else {
    ...
    async create(data) {
        return this.db.transaction(async (client) => {
            ...
            const ins = await client.query(`INSERT INTO outbound_orders
    ...
    async update(id, data, currentUserId = null) {
        await this.db.transaction(async (client) => {
            await client.query(`UPDATE outbound_orders SET
           order_date = $1,
           customer_id = $2,
           so_number = $3,
           do_number = $4,
         ...
         WHERE id = $17`, [
            ...
            if (data.shipped_date !== undefined || data.status !== undefined) {
                await client.query('UPDATE outbound_orders SET shipped_by = $1, status = $2 WHERE id = $3', [currentUserId, data.status ?? 'Open', id]);
            }
        });
    }
    async updateItem(itemId, data) {
        await this.db.query(`UPDATE outbound_items SET
    ...
    async deleteItem(itemId) {
        ...
        await db.transaction(async (client) => {
            const itemR = await client.query(`SELECT oi.*, oo.status AS order_status, p.uom_per_pallet
         FROM outbound_items oi
         ...
         WHERE oi.id = $1`, [itemId]);
            ...
            if (!item) {
                ...
                if (destInfo?.destination_id) {
                    const remaining = await client.query('SELECT COUNT(*)::int FROM outbound_items WHERE destination_id = $1', [destInfo.destination_id]);
            ...
            if (wasPicked && qty > 0 && !isUnserv) {
                const obLocs = await client.query(`SELECT oil.quantity AS restore_qty,
           ...
           WHERE oil.outbound_item_id = $1`, [itemId]);
                for (const pr of obLocs.rows) {
                    ...
                    if (!restored && rLoc) {
                        const find = await client.query('SELECT id FROM stock WHERE product_id = $1 AND batch_number IS NOT DISTINCT FROM $2 AND location = $3 AND stock_status = \'Available\'', [pid, rBatch, rLoc]);
                        ...
                        if (found) {
                            ...
                            const ins = await client.query(`INSERT INTO stock (product_id, batch_number, location, quantity, uom, pallet, stock_status)
                 VALUES ($1,$2,$3,$4,$5,$6,'Available') RETURNING id`, [pid, rBatch, rLoc, rQty, item.uom ?? 'Drum', Math.max(1, Math.ceil(rQty / uomPerPallet))]);
    ...
    async changeItemStatus(itemId, newSt) {
        const db = this.db;
        if (!['Goods Received', 'ATP', 'Unserviceable'].includes(newSt)) {
            throw api_exception_1.ApiException.badRequest('Status tidak valid.');
        }
        const itR = await db.query(`SELECT oi.*, oo.status AS order_status
       FROM outbound_items oi JOIN outbound_orders oo ON oo.id = oi.outbound_order_id WHERE oi.id = $1`, [itemId]);
        const it = itR.rows[0];
        if (!it)
            throw api_exception_1.ApiException.notFound('Item tidak ditemukan');
        if (newSt === 'ATP' && (it.in_process_status ?? '') !== 'ATP') {
            const stockCheck = await db.query(`SELECT COUNT(*)::int FROM stock
         WHERE product_id = $1 AND batch_number IS NOT DISTINCT FROM COALESCE($2,$3)
           AND stock_status = 'Available' AND quantity > 0
           AND (location IS NULL OR location NOT IN ('QUA_SHELL','STAGING'))`, [it.product_id, it.batch_number ?? null, it.batch_no ?? null]);
            if (Number(stockCheck.rows[0].count) === 0) {
                throw api_exception_1.ApiException.conflict('Inbound belum ATP. Stock belum tersedia untuk item ini.');
            }
        }
        await db.transaction(async (client) => {
            if (newSt === 'Unserviceable') {
                if (['Picking', 'Shipped'].includes(it.order_status ?? '')) {
                    ...
                    if (loc && loc !== 'QUA_SHELL') {
                        await client.query(`UPDATE stock SET quantity = GREATEST(0, quantity - $1)
               WHERE product_id=$2 AND batch_number IS NOT DISTINCT FROM $3 AND location=$4 AND stock_status='Available'`, [qty, it.product_id, batch, loc]);
                        await client.query(`DELETE FROM stock WHERE quantity<=0 AND location=$1 AND stock_status='Available'`, [loc]);
                    }
                    const upsert = await client.query(`SELECT id FROM stock WHERE product_id=$1 AND batch_number IS NOT DISTINCT FROM $2 AND location='QUA_SHELL' AND stock_status='Rejected' LIMIT 1`, [it.product_id, batch]);
                    if (upsert.rows[0]) {
                        await client.query(`UPDATE stock SET quantity = quantity + $1, pallet = pallet + $2 WHERE id = $3`, [qty, Number(it.pallet ?? 0), upsert.rows[0].id]);
                    }
                    else {
                        await client.query(`INSERT INTO stock (product_id, batch_number, location, quantity, uom, pallet, stock_status)
               VALUES ($1,$2,'QUA_SHELL',$3,$4,$5,'Rejected')`, [it.product_id, batch, qty, it.uom ?? 'Drum', Number(it.pallet ?? 0)]);
                    }
            ...
            if (newSt === 'ATP' && (it.in_process_status ?? '') === 'Unserviceable') {
                ...
                const qty = Number(it.actual_qty ?? it.quantity ?? 0);
                await client.query(`UPDATE stock SET quantity=GREATEST(0,quantity-$1)
           WHERE product_id=$2 AND batch_number IS NOT DISTINCT FROM $3 AND location='QUA_SHELL' AND stock_status='Rejected'`, [qty, it.product_id, batch]);
    ...
    async pickItems(outboundId) {
        const db = this.db;
        await db.transaction(async (client) => {
            const outbound = await this.getById(outboundId);
            if (!outbound)
                throw api_exception_1.ApiException.notFound('Outbound tidak ditemukan');
            if (outbound.status !== 'Open')
                throw api_exception_1.ApiException.conflict('Hanya order berstatus Open yang bisa di-pick');
            const items = await this.getItems(outboundId);
            const pickLocs = Array.from(new Set(items.map((it) => it.location).filter(Boolean)));
            if (pickLocs.length > 0) {
                const locked = await client.query(`SELECT st.take_number, sti.location
           ...
           LIMIT 5`, pickLocs);
                ...
                if (rows.length > 0) {
                    ...
                    const locs = Array.from(new Set(rows.map((r) => r.location))).join(', ');
                    throw api_exception_1.ApiException.conflict(`Picking diblokir — lokasi [${locs}] sedang dalam sesi Stock Take aktif (${takeNo}). Selesaikan atau batalkan stock take terlebih dahulu.`);
                }
            }
           ...
           ORDER BY
             ...
             id ASC`, [productId]);
                ...
                if (preferBatch) {
                    stockRows = stockRows
                        .map((s) => ({ s, match: s.batch_number === preferBatch ? 0 : 1 }))
                        .sort((a, b) => {
                        if (a.match !== b.match)
                ...
                if (!stockRows.length) {
                    throw api_exception_1.ApiException.conflict('Stok tidak tersedia untuk produk: ' + (item.product_name ?? productId));
                }
                const totalAvail = stockRows.reduce((a, s) => a + Number(s.quantity), 0);
                if (totalAvail < needed - 0.001) {
                    throw api_exception_1.ApiException.conflict('Stok kurang ' + (needed - totalAvail).toFixed(2) +
                ...
                for (const stock of stockRows) {
                    ...
                    if (newQty <= 0.001) {
                        ...
                        await client.query(`UPDATE stock SET quantity = $1, pallet = $2, stock_status = 'Available', updated_at = NOW() WHERE id = $3`, [newQty, Number(newPlt.toFixed(4)), stock.id]);
                    }
                    const slRow = await client.query(`SELECT id, quantity FROM stock_locations WHERE stock_id = $1 AND status = 'Available' ORDER BY pallet_seq ASC LIMIT 1`, [stock.id]);
                    let sl = slRow.rows[0];
                    let slId = null;
                    if (sl) {
                        slId = Number(sl.id);
                        const slNewQty = Math.max(0, Number(sl.quantity) - deduct);
                        await client.query(`UPDATE stock_locations SET quantity = $1, status = $2 WHERE id = $3`, [slNewQty, slNewQty <= 0 ? 'Picked' : 'Available', slId]);
                    }
                    else {
                        const slFind = await client.query(`SELECT sl.id, sl.quantity
               ...
               ORDER BY (sl.status='Available') DESC, sl.pallet_seq ASC LIMIT 1`, [productId, stock.batch_number ?? null, stock.location ?? null]);
                        ...
                        if (sl) {
                            ...
                            const insSl = await client.query(`INSERT INTO stock_locations (stock_id, location_code, pallet_seq, quantity, original_quantity, uom, is_full_pallet, batch_number, inbound_item_id, status)
                 VALUES (NULL, $1, 999, 0, $2, $3, 0, $4, NULL, 'Picked') RETURNING id`, [stock.location ?? 'UNALLOCATED', deduct, stock.uom ?? (item.uom ?? 'EA'), stock.batch_number ?? null]);
                ...
                for (const pr of pickedRows) {
                    if (pr.stock_location_id) {
                        await client.query(`INSERT INTO outbound_item_locations (outbound_item_id, stock_location_id, quantity)
               VALUES ($1,$2,$3) ON CONFLICT DO NOTHING`, [item.id, pr.stock_location_id, pr.quantity]);
                    }
                }
                await client.query(`UPDATE outbound_items SET batch_no = $1, batch_number = $1, location = COALESCE($2, location), exp_date = $3 WHERE id = $4`, [usedBatch, usedLocation, stockRows[0].expiry_date ?? null, item.id]);
            }
            await client.query('UPDATE outbound_orders SET status = \'Picking\', updated_at = NOW() WHERE id = $1', [outboundId]);
        });
    }
    async ship(outboundId, currentUserId = null) {
        const db = this.db;
        await db.transaction(async (client) => {
            const outbound = await this.getById(outboundId);
            if (!outbound)
                throw api_exception_1.ApiException.notFound('Outbound tidak ditemukan');
            const items = await this.getItems(outboundId);
            await client.query(`UPDATE outbound_orders SET status = 'Shipped', shipped_by = $1, shipped_date = $2, updated_at = NOW() WHERE id = $3`, [currentUserId, (0, date_util_1.todayStr)(), outboundId]);
            for (const item of items) {
                const qty = Number(item.actual_qty ?? item.quantity ?? 0);
                if (qty <= 0)
                    continue;
    ...
    async complete(outboundId) {
        const db = this.db;
        const current = await this.getById(outboundId);
        if (!current)
            throw api_exception_1.ApiException.notFound('Outbound tidak ditemukan.');
        if (current.status === 'Cancelled') {
            throw api_exception_1.ApiException.conflict('Order sudah dibatalkan dan tidak dapat diselesaikan.');
        }
    ...
    async delete(outboundId) {
        ...
        await db.transaction(async (client) => {
            ...
            if (['Completed', 'Cancelled', 'Shipped', 'Delivered'].includes(outbound.status ?? '')) {
                throw api_exception_1.ApiException.conflict('Order sudah ' + (outbound.status ?? '') + ' dan tidak dapat dihapus.');
            }
            const items = await this.getItems(outboundId);
            ...
            for (const item of items) {
                ...
                if (wasPickedOrShipped && qty > 0 && !isUnserv) {
                    const obLocs = await client.query(`SELECT oil.quantity AS restore_qty, sl.id AS sl_id, sl.stock_id,
             ...
             WHERE oil.outbound_item_id = $1`, [item.id]);
                    ...
                    if (pickRows.length > 0) {
                        for (const pr of pickRows) {
                            ...
                            if (!restored && rLoc) {
                                const find = await client.query(`SELECT id, quantity FROM stock WHERE product_id = $1 AND batch_number IS NOT DISTINCT FROM $2 AND location = $3 AND stock_status = 'Available'`, [pid, rBatch, rLoc]);
                                const found = find.rows[0];
                                if (found) {
                                    await client.query('UPDATE stock SET quantity = quantity + $1, updated_at = NOW() WHERE id = $2', [rQty, found.id]);
                                    if (slId) {
                                    ...
                                    const uomPerPallet = Math.max(1, Number(item.uom_per_pallet ?? 4));
                                    const ins = await client.query(`INSERT INTO stock (product_id, batch_number, location, quantity, uom, pallet, expiry_date, stock_status)
                     VALUES ($1,$2,$3,$4,$5,$6,$7,'Available') RETURNING id`, [pid, rBatch, rLoc, rQty, item.uom ?? 'Drum', Math.max(1, Math.ceil(rQty / uomPerPallet)), item.exp_date ?? item.expiry_date ?? null]);
                        ...
                        if (loc && loc !== 'QUA_SHELL' && qty > 0) {
                            const find = await client.query(`SELECT id FROM stock WHERE product_id = $1 AND batch_number IS NOT DISTINCT FROM $2 AND location = $3 AND stock_status = 'Available'`, [pid, batch, loc]);
                            ...
                            if (row) {
                                ...
                                const uomPerPallet = Math.max(1, Number(item.uom_per_pallet ?? 4));
                                await client.query(`INSERT INTO stock (product_id, batch_number, location, quantity, uom, pallet, stock_status)
                   VALUES ($1,$2,$3,$4,$5,$6,'Available')`, [pid, batch, loc, qty, item.uom ?? 'Drum', Math.max(1, Math.ceil(qty / uomPerPallet))]);
                            }
                        }
                    }
                }
             ...
             AND location='QUA_SHELL' AND stock_status='Rejected'`, [pid, batch]);
                ...
                await client.query(`DELETE FROM stock_ledger WHERE reference_type='Outbound' AND reference_id=$1 AND product_id=$2`, [outboundId, pid]);
                await client.query('DELETE FROM outbound_item_locations WHERE outbound_item_id = $1', [item.id]);
            }
            ...
            await client.query(`DELETE FROM picklist_items pi USING picklists pl WHERE pi.picklist_id = pl.id AND pl.outbound_order_id = $1`, [outboundId]);
    ...
    async getStats() {
        const db = this.db;
        const stats = {};
        const month = await db.query(`SELECT COUNT(*)::int as count FROM outbound_orders
...
exports.OutboundService = OutboundService = __decorate([
    ...
    __metadata("design:paramtypes", [db_service_1.DbService, activity_logger_1.ActivityLogger, master_data_service_1.MasterDataService])
], OutboundService);
