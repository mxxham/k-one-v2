...
let OutboundActions = class OutboundActions {
    db;
    outbound;
    activity;
    master;
    lock;
    constructor(db, outbound, activity, master, lock) {
        this.db = db;
        this.outbound = outbound;
        this.activity = activity;
        this.master = master;
        this.lock = lock;
        (0, registry_1.registerActions)('outbound', {
            list: (c) => this.list(c),
    ...
    async create(ctx) {
        ...
        for (const item of rawItems) {
            ...
            if (avail <= 0) {
                const pRow = await this.db.query('SELECT product_code, product_name FROM products WHERE id = $1', [pid]);
                const p = pRow.rows[0];
        ...
        if (rawItems.length > 0 && validItems.length === 0) {
            throw api_exception_1.ApiException.conflict('Semua produk tidak ada di stok. Order tidak dibuat. Dilewati: ' + skippedItems.join(', '));
        }
        ...
        if (Array.isArray(data.destinations) && data.destinations.length > 0) {
            ...
            await this.outbound.saveDestinations(id, d.map((x) => x.ship_to_name), d.map((x) => x.ship_to_location), d.map((x) => x.ship_to_street), d.map((x) => x.kota), d.map((x) => x.notes));
        }
        await this.activity.log('CREATE_OUTBOUND', 'outbound', 'Outbound', id, null, 'Buat outbound, customer ID ' + (data.customer_id ?? '—') + ', SO: ' + (data.so_number ?? '—'), null, null, this.actCtx(ctx));
    ...
    async update(ctx) {
        ...
        if ((obCheck?.status ?? '') === 'Completed') {
            throw api_exception_1.ApiException.conflict('Order sudah Completed dan tidak dapat diedit.');
        }
    ...
    async delete(ctx) {
        ...
        if (['Completed', 'Cancelled', 'Shipped', 'Delivered'].includes(ob.status ?? '')) {
            throw api_exception_1.ApiException.conflict('Order sudah ' + ob.status + ' dan tidak dapat dihapus.');
        }
        await this.outbound.delete(id);
        await this.activity.log('DELETE_OUTBOUND', 'outbound', 'Outbound', id, ob.order_number ?? null, 'Hapus outbound ' + (ob.order_number ?? id), null, null, this.actCtx(ctx));
        return { id };
    }
    async addItem(ctx) {
        const data = ctx.body;
        ...
        try {
            ...
            if (e instanceof api_exception_1.ApiException && e.getStatus?.() === 409)
                throw e;
            throw api_exception_1.ApiException.conflict(e.message);
        }
        await this.outbound.attachDestination(outboundId, newItemId, item);
        await this.activity.log('ADD_OUTBOUND_ITEM', 'outbound', 'Outbound', outboundId, null, 'Tambah item produk ID ' + (item.product_id ?? '?') + ' qty ' + (item.quantity ?? 0), null, null, this.actCtx(ctx));
    ...
    async pickItems(ctx) {
        ...
        if (String(ob.expected_date ?? '').trim() === '') {
            throw api_exception_1.ApiException.conflict('Expected Date wajib diisi sebelum Pick Items.');
        }
        await this.lock.runLocked(shared_1.LOCK_KEYS.fefo(id), () => this.outbound.pickItems(id), { ttlMs: 60_000 });
        await this.activity.log('PICK_OUTBOUND', 'outbound', 'Outbound', id, ob.order_number ?? null, 'Pick outbound ' + (ob.order_number ?? id), null, null, this.actCtx(ctx));
        return { id };
    }
    async ship(ctx) {
        const data = ctx.body;
        const id = Number.parseInt(data.id ?? ctx.query.id ?? '0', 10) || 0;
        const ob = await this.outbound.getById(id);
        await this.lock.runLocked(shared_1.LOCK_KEYS.fefo(id), () => this.outbound.ship(id, ctx.user.id), { ttlMs: 60_000 });
        await this.activity.log('SHIP_OUTBOUND', 'outbound', 'Outbound', id, ob?.order_number ?? null, 'Kirim outbound ' + (ob?.order_number ?? id), null, null, this.actCtx(ctx));
        return { id };
    }
    async complete(ctx) {
        const data = ctx.body;
        ...
        await this.activity.log('COMPLETE_OUTBOUND', 'outbound', 'Outbound', id, ob?.order_number ?? null, 'Selesai outbound ' + (ob?.order_number ?? id), null, null, this.actCtx(ctx));
        return { id };
    }
    async deleteItem(ctx) {
        ...
        await this.activity.log('DELETE_OUTBOUND_ITEM', 'outbound', 'Outbound', outboundId, null, `Hapus item ID ${itemId} dari outbound ID ${outboundId}`, null, null, this.actCtx(ctx));
    ...
    async updateItemStatus(ctx) {
        ...
        await this.activity.log('UPDATE_OB_ITEM_STATUS', 'outbound', 'Outbound', outboundId, null, `Status outbound item ID ${itemId} → ${status}`, null, null, this.actCtx(ctx));
        return { item_id: itemId, status };
    }
};
exports.OutboundActions = OutboundActions;
exports.OutboundActions = OutboundActions = __decorate([
...
//# sourceMappingURL=outbound.actions.js.map