/**
 * HTML print document rendering mirroring the legacy PHP print pages:
 *  - print_inbound.php   -> inbound receipt (Goods Receipt Document)
 *  - putaway_sheet.php   -> putaway sheet
 *  - print_outbound.php  -> outbound DO
 *  - surat_jalan.php     -> delivery note
 *  - print_picklist.php  -> picklist print
 *  - print_report.php    -> daily / stock / expiring report print
 * The gateway streams these with _html (inline text/html), matching the
 * browser-print behavior of the PHP pages.
 */
...
function fmtLong(d: unknown): string {
  if (!d) return '';
...
function fmtDmy(d: unknown): string {
  if (!d) return '';
  const m = /^(\d{4})-(\d{2})-(\d{2})/.exec(String(d));
  if (!m) return String(d);
  return `${m[3]}/${m[2]}/${m[1]}`;
...
function monthsAgo3(dateStr: unknown): boolean {
...
export class PrintService {
  ...
  // ---------------------------------------------------------------------------
  // Inbound receipt (print_inbound.php)
  // ---------------------------------------------------------------------------
  async inboundReceipt(id: number): Promise<string> {
    const inbound = await this.inbound.getById(id);
    if (!inbound) throw ApiException.badRequest('Inbound tidak ditemukan');
    const items = await this.inbound.getItems(id);

    const itemLocations: Record<string, any[]> = {};
    for (const item of items) {
      const locs = await this.inbound.getItemLocations(Number(item.id));
      if (locs.length > 0) itemLocations[String(item.id)] = locs;
    }
...
<div class="document">
  ...
  // ---------------------------------------------------------------------------
  // Putaway sheet (putaway_sheet.php)
  // ---------------------------------------------------------------------------
  async putawaySheet(id: number): Promise<string> {
    const inbound = await this.inbound.getById(id);
    if (!inbound) throw ApiException.badRequest('Inbound tidak ditemukan');
    const items = await this.inbound.getItems(id);

    const putawayRows: any[] = [];
    let totalQty = 0;
    let totalLines = 0;
    for (const item of items) {
      const locs = await this.inbound.getItemLocations(Number(item.id));
...
<div class="document">
  ...
  // ---------------------------------------------------------------------------
  // Outbound DO (print_outbound.php)
  // ---------------------------------------------------------------------------
  async outboundDo(id: number): Promise<string> {
    const outbound = await this.outbound.getById(id);
    if (!outbound) throw ApiException.badRequest('Outbound tidak ditemukan');
    ...
    const calcItemPallet = (item: any) => Math.ceil(Number(item.actual_qty ?? item.quantity ?? 0) / Math.max(1, Number(item.uom_per_pallet ?? 4)));

    const itemPickLocations: Record<string, any[]> = {};
    for (const item of items) {
      const locs = await this.outbound.getItemPickedLocations(Number(item.id));
      if (locs.length > 0) itemPickLocations[String(item.id)] = locs;
    }

    const itemsByDest: Record<number, any[]> = {};
    for (const item of items) {
      const dId = item.destination_id ? Number(item.destination_id) : 0;
      if (!itemsByDest[dId]) itemsByDest[dId] = [];
...
<div class="document">
  ...
  // ---------------------------------------------------------------------------
  // Surat jalan (surat_jalan.php)
  // ---------------------------------------------------------------------------
  async suratJalan(id: number): Promise<string> {
    const outbound = await this.outbound.getById(id);
    if (!outbound) throw ApiException.badRequest('Outbound tidak ditemukan');
    const items = await this.outbound.getItems(id);
    const destinations = await this.outbound.getDestinations(id);
    ...
    const status = String(outbound.status ?? 'Open');
    const badgeColors: Record<string, string> = { Completed: '#013d3c', Shipped: '#014f4e', Picked: '#026766', Open: '#e65100' };
    ...
    const rowsHtml = items
      .map((item, i) => {
        ...
        for (const pl of pickLocs) if (pl.location_code && !uniqLocs.includes(pl.location_code)) uniqLocs.push(pl.location_code);
...
<div class="document">
  ...
  // ---------------------------------------------------------------------------
  // Picklist print (print_picklist.php)
  // ---------------------------------------------------------------------------
  async picklist(id: number): Promise<string> {
    const picklist = await this.picklistService.getById(id);
    if (!picklist) throw ApiException.badRequest('Picklist tidak ditemukan');
...
<div class="document">
  ...
  // ---------------------------------------------------------------------------
  // Report print (print_report.php): type=daily|stock|expiring
  // ---------------------------------------------------------------------------
  async reportPrint(type: string, date: string | null, dateTo: string | null): Promise<string> {
    ...
    if (reportType === 'daily' && report) {
      ...
      <table>
        ...
        <tbody>${(report.stock_summary ?? []).map((item: any) => `<tr>
        ...
        <tfoot><tr>
          <td colspan="4"><strong>TOTAL</strong></td>
          <td class="r">${numFmt((report.stock_summary ?? []).reduce((s: number, it: any) => s + Number(it.total_drums ?? it.total_qty ?? 0), 0))}</td>
          <td class="r">${Math.ceil((report.stock_summary ?? []).reduce((s: number, it: any) => s + Number(it.total_pallets ?? 0), 0))}</td>
          <td></td>
        </tr></tfoot>
      </table>`;
      if (report.inbound_activity?.length) {
        ...
        <table>
          ...
          <tbody>${report.inbound_activity.map((item: any) => {
      ...
      if (report.outbound_activity?.length) {
        ...
        <table>
          ...
          <tbody>${report.outbound_activity.map((item: any) => {
            const st = String(item.status ?? '');
      ...
      if (report.expiring_items?.length) {
        ...
        <table>
          ...
          <tbody>${report.expiring_items.map((item: any) => {
      ...
      <table>
        ...
        <tfoot><tr>
          ...
          <td class="r">${numFmt(stock.reduce((s: number, it: any) => s + Number(it.quantity_in ?? it.quantity_drums ?? 0), 0))}</td>
          <td class="r">${Math.ceil(stock.reduce((s: number, it: any) => s + Number(it.pallet ?? 0), 0))}</td>
          <td colspan="3"></td>
        </tr></tfoot>
      </table>`;
...
<div class="document">
  ...
  private async stockAllInternal(): Promise<any[]> {
    return this.report.reportStock();
  }

  private async expiringInternal(): Promise<any[]> {
